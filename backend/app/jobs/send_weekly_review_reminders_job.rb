...
class SendWeeklyReviewRemindersJob < ApplicationJob
  queue_as :default

  def perform
    User.joins(:notification_preference)
        .where(notification_preferences: { email: true, weekly_review: true })
        .find_each do |user|
  ...
  def send_reminder_if_due(user)
    ...
    # Skip if check-in frequency doesn't include weekly reminders
    return unless preference.weekly_reminders_enabled?

    timezone = family.timezone || "UTC"
    current_time = Time.current.in_time_zone(timezone)

    # Check if today is the weekly review day
    return unless current_time.wday == preference.weekly_review_day

    # Check if now is the scheduled time (within 5 minute window)
    scheduled_time = parse_time(preference.weekly_review_time, timezone, current_time)
    ...
    # Send the email
    ReminderMailer.weekly_review(user, family).deliver_later
  end
