...
class SendEveningReflectionRemindersJob < ApplicationJob
  queue_as :default

  def perform
    User.joins(:notification_preference)
        .where(notification_preferences: { email: true, evening_reflection: true })
        .find_each do |user|
  ...
  def send_reminder_if_due(user)
    ...
    scheduled_time = parse_time(preference.evening_reflection_time, timezone, current_time)
    ...
    # Send the email
    ReminderMailer.evening_reflection(user, family).deliver_later
  end
