...
type Props = RootStackScreenProps<"CreateGoal">;
...
const STATUSES: GoalStatus[] = ["not_started", "in_progress", "at_risk"];
...
export function CreateGoalScreen({ navigation }: Props) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const newGoal = await createGoal(familyId, data);
      navigation.replace("GoalDetail", { goalId: newGoal.id.toString() });
    } catch (error) {
  ...
  const handleRefineWithAI = async () => {
    if (!familyId || !title.trim()) {
      Alert.alert("Error", "Please enter a title before refining");
      return;
    }

    setIsRefining(true);
    try {
      // Create a temporary goal to get AI suggestions
      const tempData: CreateGoalData = {
        title: title.trim(),
        description: description.trim() || undefined,
        time_scale: timeScale,
        status: "not_started",
      ...
      const tempGoal = await createGoal(familyId, tempData);
      const refinement = await refineGoal(tempGoal.id);

      // Apply suggestions to the form
      if (refinement.smart_suggestions.specific && !specific.trim()) {
        setSpecific(refinement.smart_suggestions.specific);
      }
      if (refinement.smart_suggestions.measurable && !measurable.trim()) {
        setMeasurable(refinement.smart_suggestions.measurable);
      }
      if (refinement.smart_suggestions.achievable && !achievable.trim()) {
        setAchievable(refinement.smart_suggestions.achievable);
      }
      if (refinement.smart_suggestions.relevant && !relevant.trim()) {
        setRelevant(refinement.smart_suggestions.relevant);
      }
      if (refinement.smart_suggestions.time_bound && !timeBound.trim()) {
        setTimeBound(refinement.smart_suggestions.time_bound);
      }

      // Navigate to the created goal
      navigation.replace("GoalDetail", { goalId: tempGoal.id.toString() });
    } catch (error) {
      console.error("Failed to refine:", error);
      Alert.alert("Error", "AI coaching is temporarily unavailable");
    } finally {
      setIsRefining(false);
    }
  };
      ...
      <View style={styles.smartSection}>
        <View style={styles.smartHeader}>
          ...
          >
            {isRefining ? (
              ...
              <Text style={styles.refineButtonText}>✨ AI Help</Text>
            )}
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.field}>
          ...
          <TextInput
            ...
            placeholder="Be clear and detailed..."
            placeholderTextColor={`${COLORS.earthBrown}60`}
            multiline
          />
        </View>
        ...
        <View style={styles.field}>
          ...
          <Text style={styles.smartHint}>How will you track progress?</Text>
          <TextInput
            style={[styles.textInput, styles.smartInput]}
            value={measurable}
            onChangeText={setMeasurable}
            placeholder="Numbers, metrics, milestones..."
            placeholderTextColor={`${COLORS.earthBrown}60`}
            multiline
          />
        </View>
        ...
        <View style={styles.field}>
          ...
          <Text style={styles.smartHint}>
            Is this realistic with your resources?
          </Text>
          <TextInput
            style={[styles.textInput, styles.smartInput]}
            value={achievable}
            onChangeText={setAchievable}
            placeholder="Steps and resources needed..."
            placeholderTextColor={`${COLORS.earthBrown}60`}
            multiline
          />
        </View>
        ...
        <View style={styles.field}>
          ...
          <Text style={styles.smartHint}>Why does this matter to you?</Text>
          <TextInput
            style={[styles.textInput, styles.smartInput]}
            value={relevant}
            onChangeText={setRelevant}
            placeholder="Connection to bigger goals..."
            placeholderTextColor={`${COLORS.earthBrown}60`}
            multiline
          />
        </View>
        ...
        <View style={styles.field}>
          ...
          <Text style={styles.smartHint}>When will you complete this?</Text>
          <TextInput
            style={[styles.textInput, styles.smartInput]}
            value={timeBound}
            onChangeText={setTimeBound}
            placeholder="Deadline and milestones..."
            placeholderTextColor={`${COLORS.earthBrown}60`}
            multiline
          />
        </View>
